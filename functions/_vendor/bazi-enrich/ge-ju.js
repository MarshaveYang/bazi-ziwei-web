...
export function judgeGeJu(siZhu) {
    ...
    if (isPure) {
        ...
        // 看月令藏干中是否有透出
        const touSS = tougan
            .map(g => ({ gan: g, ss: (0, tables_1.getShiShen)(dm, g) }))
    ...
    if (benqiSS === '比肩') {
        ...
        confidence = '高';
        notes.length = 0;
        const cs = (0, tables_1.getChangSheng)(dm, monthZhi);
        if (cs === '临官') {
        ...
        const yy = tables_1.GAN_YINYANG[dm];
        notes.length = 0;
        primary = '劫财格';
