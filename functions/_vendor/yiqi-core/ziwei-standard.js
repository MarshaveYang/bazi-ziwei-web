...
export function createZiweiChart(birthInfo) {
    try {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            const dizhi = DIZHI[gongZhiIndex];
            // 计算天干（使用五虎遁月诀）
            const tiangan = getMonthGan(yearGan, dizhi);
            // 找出这个宫位的主星
            const mainStars = [];
        ...
        // 11. 添加本命四化（出生年干的四化）
        const gongsWithSiHua = (0, sihua_1.addBenMingSiHua)(gongs, yearGan);
        // 12. 计算大运信息
        const currentAge = (0, daxian_1.calculateXuSui)(birthInfo.year);
        const daXianList = (0, daxian_1.calculateDaXian)(mingGongIndex, yinYang, wuXingJu.number);
        // 13. 为每个宫位添加大运信息和流年
        // 找到当前大限
