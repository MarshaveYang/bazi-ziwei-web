...
export function createBaziChart(birthInfo) {
    try {
        ...
        const siZhu = {
            ...
            day: {
                gan: baZiForDay.getDay().substring(0, 1), // 使用调整后的日柱（处理晚子时）
                zhi: baZiForDay.getDay().substring(1, 2) // 使用调整后的日柱（处理晚子时）
            },
            hour: {
                gan: baZi.getTime().substring(0, 1),
                zhi: baZi.getTime().substring(1, 2)
            }
        };
        ...
        const shiShen = {
            ...
            day: getShiShen(dayMaster, siZhu.day.gan),
        ...
        const dayun = dayunList.map((d) => {
            const ganZhiStr = d.getGanZhi();
            const ganZhi = {
                gan: ganZhiStr.substring(0, 1),
                zhi: ganZhiStr.substring(1, 2)
            };
            // 计算天干十神
            const ganShiShen = getShiShen(dayMaster, ganZhi.gan);
            // 计算地支藏干的十神
            const zhiCangGan = getZhiCangGan(ganZhi.zhi);
            const zhiShiShen = getShiShen(dayMaster, zhiCangGan);
            // 获取该大运内的流年
            const liuNianList = d.getLiuNian();
            const liuNian = liuNianList.map((ln) => {
                const lnGanZhiStr = ln.getGanZhi();
                return {
                    year: ln.getYear(),
                    age: ln.getAge(),
                    ganZhi: {
                        gan: lnGanZhiStr.substring(0, 1),
                        zhi: lnGanZhiStr.substring(1, 2)
                    }
                };
            });
            return {
        ...
        const zhangSheng = (0, zhangsheng_1.getSiZhuChangSheng)(dayMaster, {
            year: siZhu.year.zhi,
        ...
        throw new Error('八字排盘计算失败: ' + error.message);
    }
}
/**
 * 运行八字测试
    ...
    const testCases = [
        {
            ...
            input: { year: 1979, month: 5, day: 4, hour: 6, minute: 0, gender: 'male', isLunar: false, timeZone: 8 },
            expected: {
                year: '己未',
                month: '戊辰',
                day: '辛未',
        ...
        },
        {
            ...
            input: { year: 2000, month: 1, day: 1, hour: 12, minute: 0, gender: 'female', isLunar: false, timeZone: 8 },
            expected: {
                year: '己卯',
                month: '丙子',
                day: '戊午',
        ...
        },
        {
            ...
            input: { year: 1988, month: 8, day: 15, hour: 10, minute: 0, gender: 'male', isLunar: false, timeZone: 8 },
            expected: {
        ...
        },
        {
            ...
            input: { year: 1987, month: 3, day: 20, hour: 14, minute: 0, gender: 'female', isLunar: false, timeZone: 8 },
            expected: {
        ...
        },
        {
            ...
            input: { year: 2000, month: 3, day: 1, hour: 12, minute: 0, gender: 'female', isLunar: false, timeZone: 8 },
            expected: {
                year: '庚辰',
                month: '戊寅',
                day: '乙卯',
